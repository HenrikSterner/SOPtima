...
_TEACHER_INITIALS_PATTERN = re.compile(r"^[a-z]{2,6}$")
...
def default_teacher_teams_email(teacher_id: Any) -> str:
    ...
    if identifier in KNOWN_TEACHER_EMAILS:
        return KNOWN_TEACHER_EMAILS[identifier]
    if _TEACHER_INITIALS_PATTERN.fullmatch(identifier):
        return f"{identifier}@nextkbh.dk"
    return ""
...
def _student_email_column(columns: list[Any]) -> Any | None:
    """Find elevens Teams-id; den generiske kolonne ``Email`` er elevens."""
    for column in columns:
        if core.normal_key(column) in {"email", "mail", "upn"}:
            return column
    return _email_column(columns, "elev", "studer")


def _exact_column(columns: list[Any], *names: str) -> Any | None:
    wanted = {core.normal_key(name) for name in names}
    return next((column for column in columns if core.normal_key(column) in wanted), None)


def _student_name_column(columns: list[Any]) -> Any | None:
    return _exact_column(columns, "Elev", "Elevnavn", "Navn", "Student")


def _student_class_column(columns: list[Any]) -> Any | None:
    return _exact_column(columns, "Klasse", "Hold", "Class")


def _schedule_table_with_student_email(uploaded: Any) -> pd.DataFrame:
    """Find samme tidsplantabel som grundparseren, også efter titelrækker/andre ark."""
    table = core.read_uploaded_table(uploaded).dropna(how="all")
    columns = list(table.columns)
    if _student_name_column(columns) is not None and _student_email_column(columns) is not None:
        return table
    filename = core.repair_text(getattr(uploaded, "name", "")).casefold()
    if not filename.endswith((".xlsx", ".xlsm")):
        return table
    sheets = pd.read_excel(BytesIO(uploaded.getvalue()), sheet_name=None, header=None, dtype=object)
    for raw in sheets.values():
        for row_number, row in raw.head(50).iterrows():
            headers = [core.repair_text(value) or f"Kolonne {index + 1}" for index, value in enumerate(row.tolist())]
            if _student_name_column(headers) is None or _student_email_column(headers) is None:
                continue
            candidate = raw.iloc[int(row_number) + 1 :].copy()
            candidate.columns = headers
            return candidate.dropna(how="all")
    return table
...
def parse_students_beta(table: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    columns = list(table.columns)
    name_col = _student_name_column(columns)
    class_col = _student_class_column(columns)
    email_col = _student_email_column(columns)
...
def parse_teachers_beta(
    ...
    for teacher in teachers:
        teacher["teams_email"] = emails.get(teacher["id"], default_teacher_teams_email(teacher["id"]))
...
def parse_schedule_upload_beta(uploaded: Any) -> dict[str, Any]:
    ...
    for teacher in parsed["teachers"]:
        teacher["teams_email"] = default_teacher_teams_email(teacher["id"])
    for student in parsed["students"]:
        student["teams_email"] = ""
    try:
        table = _schedule_table_with_student_email(uploaded)
        columns = list(table.columns)
        name_col = _student_name_column(columns)
        class_col = _student_class_column(columns)
        student_email_col = _student_email_column(columns)
...
def make_schedule_input_template_beta() -> bytes:
    columns = [
        "Elev", "Email", "Klasse", "Lærer 1", "Lærer 1 Teams-email", "Fag 1",
...
def _word_documents(schedule: dict[str, Any], progress_callback: Any = None) -> dict[str, dict[str, Any]]:
    ...
    for position, (_, row) in enumerate(frame.iterrows(), 1):
        ...
        values = {
            ...
            "Klasse": core.sop_class_label(row.get("Klasse")),
...
function wordUnxml(value){return String(value||'').replace(/&#x([0-9a-f]+);/gi,(_,code)=>String.fromCodePoint(parseInt(code,16))).replace(/&#([0-9]+);/g,(_,code)=>String.fromCodePoint(parseInt(code,10))).replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&apos;/g,"'").replace(/&amp;/g,'&');}
...
async function createWordDocument(item){
  ...
  source=source.replace(/<w:p(?:\s[^>]*)?>[\s\S]*?<\/w:p>/g,paragraph=>{
    const textPattern=/<w:t(\s[^>]*)?>([\s\S]*?)<\/w:t>/g,nodes=[...paragraph.matchAll(textPattern)];if(!nodes.length)return paragraph;
    const original=nodes.map(node=>wordUnxml(node[2])).join('');let replaced=original;
    for(const [field,value] of values)replaced=replaced.split(`<${field}>`).join(String(value||''));
    if(replaced===original)return paragraph;let first=true;
    return paragraph.replace(textPattern,(_all,attributes)=>{const content=first?wordXml(replaced):'';first=false;return `<w:t${attributes||''}>${content}</w:t>`;});
  });
